...
parfor ii = 1:2*Ntilde
    ...
    Counter(ii) = real(2*pi/N * Fg(:,ii).'*conj(gVec(:,ii)));
end
pin = 2*pi/N;
tN = (0:(N-1))*pin;
cs = cos(tN);
...
for iz = 1:size(XGrid,1)
    for jz = 1:size(XGrid,2)
        if IniValsFin(iz,jz)>=thresh
            ...
            [gCoeffMin(iz,jz,:),val(iz,jz)] = fmincon(@(gcoeff)funhandle_zReal(gcoeff,Nloc,kappaloc,Ntildeloc,zz,Nh,R,zzForG),gCoeffNloc,...
